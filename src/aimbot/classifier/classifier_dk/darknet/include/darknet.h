...
typedef enum { PNG, BMP, TGA, JPG } IMTYPE;
...
listt *read_data_cfg(char *filename);
listt *read_cfg(char *filename);
...
char *option_find_str(listt *l, char *key, char *def);
int option_find_int(listt *l, char *key, int def);
int option_find_int_quiet(listt *l, char *key, int def);
...
void get_region_detections(layer l, int w, int h, int netw, int neth,
                           float thresh, int *map, float tree_thresh,
