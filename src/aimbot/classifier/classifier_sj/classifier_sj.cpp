...
vector<vector<MatrixXd>> Classifier_sj::load_conv_w(const string &file) {
...
vector<double> Classifier_sj::load_conv_b(const string &file) {
...
MatrixXd Classifier_sj::load_fc_w(const string &file) {
    FILE *fp = fopen(file.data(), "r");
    if (fp == nullptr) {
        LOGE("%s open fail!", file.data());
        state = false;
...
VectorXd Classifier_sj::load_fc_b(const string &file) {
    FILE *fp = fopen(file.data(), "r");
    if (fp == nullptr) {
        LOGE("%s open fail!", file.data());
        state = false;
...
MatrixXd Classifier_sj::softmax(const MatrixXd &input) {
    MatrixXd tmp = input.array() - input.maxCoeff();
    return tmp.array().exp() / tmp.array().exp().sum();
}

vector<vector<MatrixXd>> Classifier_sj::max_pool(
    const vector<vector<MatrixXd>> &input, int size) {
                ...
                for (int col = 0; col < input[0][0].cols() / size; col++) {
                    ...
                    for (int x = 0; x < size; x++) {
                        for (int y = 0; y < size; y++) {
                            if (max < input[sample][channel](row * size + x,
                                                             col * size + y)) {
                                max = input[sample][channel](row * size + x,
                                                             col * size + y);
                            }
                        }
                    }
...
vector<vector<MatrixXd>> Classifier_sj::mean_pool(
    const vector<vector<MatrixXd>> &input, int size) {
    vector<vector<MatrixXd>> output;
    for (int sample = 0; sample < input.size(); sample++) {
        vector<MatrixXd> sub;
        for (int channel = 0; channel < input[0].size(); channel++) {
            ...
            for (int row = 0; row < input[0][0].rows() / size; row++) {
                for (int col = 0; col < input[0][0].cols() / size; col++) {
                    ...
                    for (int x = 0; x < size; x++) {
                        for (int y = 0; y < size; y++) {
                            val += input[sample][channel](row * size + x,
                                                          col * size + y);
                        }
                    }
...
vector<vector<MatrixXd>> Classifier_sj::apply_bias(
    const vector<vector<MatrixXd>> &input, const vector<double> &bias) {
...
MatrixXd Classifier_sj::relu(const MatrixXd &input) {
    return input.unaryExpr([](double val) { return (val > 0) ? (val) : (0); });
}

MatrixXd Classifier_sj::leaky_relu(const MatrixXd &input, float alpha) {
    return input.unaryExpr(
        [&](double val) { return (val > 0) ? (val) : (alpha * val); });
}

vector<vector<MatrixXd>> Classifier_sj::relu(
...
vector<vector<MatrixXd>> Classifier_sj::leaky_relu(
    const vector<vector<MatrixXd>> &input, float alpha) {
    vector<vector<MatrixXd>> result;
    for (int samples = 0; samples < input.size(); samples++) {
        vector<MatrixXd> sub;
        for (int channels = 0; channels < input[0].size(); channels++) {
...
vector<vector<MatrixXd>> Classifier_sj::pand(
    const vector<vector<MatrixXd>> &input, int val) {
    vector<vector<MatrixXd>> result;
    for (int sample = 0; sample < input.size(); sample++) {
        vector<MatrixXd> sub;
        for (int channels = 0; channels < input[0].size(); channels++) {
            MatrixXd mat = MatrixXd::Zero(input[0][0].rows() + 2 * val,
                                          input[0][0].cols() + 2 * val);
            mat.block(val, val, input[0][0].rows(), input[0][0].cols()) =
                input[sample][channels];
            sub.emplace_back(mat);
        }
        result.emplace_back(sub);
    }
...
MatrixXd Classifier_sj::conv(const MatrixXd &filter, const MatrixXd &input) {
    ...
    for (int row = 0; row < result_rows; row++) {
        for (int col = 0; col < result_cols; col++) {
            ...
            result(row, col) = val;
            //            result(row, col) = (input.block(row, col, size,
            //            size).array() * input.array()).sum();
        }
    }
    return result;
}

vector<vector<MatrixXd>> Classifier_sj::conv2(
    const vector<vector<MatrixXd>> &filter,
    const vector<vector<MatrixXd>> &input) {
    if (filter.size() != input[0].size()) {
        LOGE("shape du not match, which is filter.size=%d, input[0].size()=%d",
             filter.size(), input[0].size());
        exit(-1);
    }
    vector<vector<MatrixXd>> result;
...
MatrixXd Classifier_sj::flatten(const vector<vector<MatrixXd>> &input) {
...
Classifier_sj::Classifier_sj(const string &folder) : state(true) {
...
//#define PRINT_MULTI_MAT_SHAPE(name) LOGM(#name":(%d, %d)", name[0][0].rows(),
//name[0][0].cols())

MatrixXd Classifier_sj::calculate(const vector<vector<MatrixXd>> &input) {
    vector<vector<MatrixXd>> conv1_result =
        relu(apply_bias(conv2(conv1_w, input), conv1_b));
    vector<vector<MatrixXd>> pool1_result = mean_pool(conv1_result, 2);
    vector<vector<MatrixXd>> conv2_result =
        relu(apply_bias(conv2(conv2_w, pool1_result), conv2_b));
    vector<vector<MatrixXd>> pool2_result = mean_pool(conv2_result, 2);
    vector<vector<MatrixXd>> conv3_result =
        relu(apply_bias(conv2(conv3_w, pool2_result), conv3_b));
...
Classifier_sj::operator bool() const { return state; }

int Classifier_sj::operator()(const cv::Mat &image) {
    MatrixXd r, g, b;
